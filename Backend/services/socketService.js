...
async function emitAnswer(io, eventId, userId, correctAnswer) {
  ...
  const playerScore = await redis.hget(
    `eventQuizes:${eventId}:players`,
    userId
  );
  const answerObject = {
    correctAnswer,
...
async function askQuestion(io, eventId, userId, timer) {
  ...
  const question = await redis.hgetall(
    `eventQuizes:${eventId}:currentQuestion`
  );
  ...
  setTimeout(
    async (io, eventId, userId, correctAnswer, isContinuous, newQuestion) => {
      // Cập nhật lại điểm của người chơi
...
class SocketService {
  ...
  static connection(socket) {
    ...
    socket.on("playEvent", async (eventId, userId) => {
      ...
      const player = await UserService.findPlayerWithTicketOfEventById(
        userId,
        eventId
      );
    ...
    socket.on("submitAnswer", async (userId, answer) => {
      ...
      if (eventId != null) {
        ...
        if (isCorrect) {
          ...
          const point = await redis.hget(
            `eventQuizes:c4fb5c4d-4ef7-4c43-b978-4bfab379dce1:players`,
            "nhan"
          );
        }
      }
