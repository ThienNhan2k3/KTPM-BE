  ...
  try {
    ...
    if (userItems.length === 0) {
      return res
        .status(404)
        .json({
          message: "No items found for this user in the specified event",
        });
    }
...
exports.getBrandsAndEventsOfItemsByUser = async (req, res) => {
  ...
  try {
    ...
    const uniqueBrandEventPairs = brandEventPairs.filter(
      (value, index, self) =>
        index ===
        self.findIndex(
          (t) => t.id_brand === value.id_brand && t.id_event === value.id_event
        )
    );
...
exports.getGiftItemsHistoryByGiver = async (req, res) => {
  ...
  try {
    ...
    if (giftHistory.length > 0) {
      ...
      const result = giftHistory.reduce((acc, current) => {
        const existingGroup = acc.find(
          (group) =>
            group.id_recipient === current.id_recipient &&
            group.gift_time === current.gift_time.toISOString()
        );
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching gift items history" });
  }
};
